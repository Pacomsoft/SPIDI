...
import { type IDailySummaryListItemDTO, type IDailySummaryFiltersDTO } from '../../domain/contracts/daily-summary.dto';

export interface IUseDailySummariesListResult {
  summaries: IDailySummaryListItemDTO[];
  total: number;
  isLoading: boolean;
  error: string | null;
  page: number;
...
export function useDailySummariesList(
  getDailySummariesUseCase: IGetDailySummariesUseCase,
  exportDailySummariesUseCase: IExportDailySummariesUseCase,
): IUseDailySummariesListResult {
  const [summaries, setSummaries] = useState<IDailySummaryListItemDTO[]>([]);
  const [total, setTotal] = useState(0);
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [page, setPage] = useState(1);
  const [pageSize, setPageSize] = useState(25);
  const [sortBy, setSortBy] = useState('summaryDate');
  ...
  const fetchSummaries = useCallback(async () => {
    setIsLoading(true);
    setError(null);
    const filters: IDailySummaryFiltersDTO = {
      page,
      pageSize,
      sortBy,
      sortDirection,
    ...
    const result = await getDailySummariesUseCase.execute(filters);
    if (result.success && result.data) {
      setSummaries(result.data.items);
      setTotal(result.data.total);
    } else {
      setError(result.error?.message ?? 'Error al cargar resúmenes diarios');
    }
    setIsLoading(false);
  }, [page, pageSize, sortBy, sortDirection, debouncedDriverSearch, dateFrom, dateTo, getDailySummariesUseCase]);
  ...
  const handleExport = useCallback(async (format: string) => {
    ...
    const result = await exportDailySummariesUseCase.execute({ filters, format });
    if (result.success && result.data) {
      const url = URL.createObjectURL(result.data);
      const link = document.createElement('a');
      link.href = url;
      link.download = `daily_summaries_${Date.now()}.${format}`;
      link.click();
      URL.revokeObjectURL(url);
    }
    setIsExporting(false);
  }, [sortBy, sortDirection, debouncedDriverSearch, dateFrom, dateTo, exportDailySummariesUseCase]);
